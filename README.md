...
Por defecto el correo se envía desde `onboarding@resend.dev` (dominio de prueba de Resend, no requiere configuración). Para enviarlo desde un dominio propio (ej. `cristian@estudiofrontier.com`) hay que verificar el dominio en Resend y cambiar el campo `from` en `web/api/contact.js`.
